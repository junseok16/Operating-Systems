 ...
 * Homework 02	: Multi-threaded Word Count; 1-producer n-consumer line read with statistic.
 * Author	: junseok Tak
 * Student ID	: 32164809
 * Date		: 2021-10-25
...
int main(int argc, char* argv[]) {
    ...
    if (argv[2] != NULL) {
        ...
        if (PRODUCER_NUMBER > 1) PRODUCER_NUMBER = 1;
    ...
    for (int i = 0; i < CONSUMER_NUMBER; i++) {
        ...
        printf("\tConsumer No.%d joined with %d lines.\n", i, *ret);
    }
    for (int i = 0; i < PRODUCER_NUMBER; i++) {
        rc = pthread_join(prod[i], (void**)&ret);
        printf("\tProducer No.%d joined with %d lines.\n", i, *ret);
    }
...
void* Producer(void* arg) {
    ...
    free(readLine);
    printf("\tProducer writes %d line(s).\n", lineIndex);
    *ret = lineIndex;
    pthread_exit(ret);
}
...
void* Consumer(void* arg) {
    ...
    while (true) {
        ...
        // collect statistic infomation from the line.
    ...
    printf("\tConsumer reads %d line(s).\n", lineIndex);
    *ret = lineIndex;
    pthread_exit(ret);
}
